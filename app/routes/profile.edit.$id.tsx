...
export async function action({ params, request }: ActionFunctionArgs) {
    ...
    if (typeof full_name !== "string" || typeof avatar_url !== "string") return null;
...
export default function Profile() {
    ...
    return (
        <div className="m-auto md:w-1/2 w-3/4 py-14">
            <h1 className="font-bold text-3xl text-center">{data.userData.full_name}のユーザー情報を編集</h1>

            <Form method="post" className="py-10">
                <div>
                    <label htmlFor="name">ユーザー名</label>
                    <Input type="text" name="full_name" id="full_name" defaultValue={data.userData.full_name} required />
                </div>
                <div>
